...
class RequirementDaoIntegrationTest extends AssetFixture {
    @Test void roundTripOrderingImmutableIdentityAndOptimisticLock() {
        tx.inTransaction(c -> {
            var p = parents(c); var dao = new JdbcRequirementDao(c);
            var old = dao.insert(requirement(p, 2));
            var first = dao.insert(requirement(p, 1));
            var otherProject = new JdbcProjectDao(c).insert(project("OTHER", p.userId()));
            dao.insert(requirement(new Parents(p.userId(), otherProject.id()), 1));
            ...
            var saved = dao.update(new Requirement(old.id(), -1L, 99L, "edited", "description", Priority.HIGH, RequirementStatus.ACTIVE, -1L, null, null, old.lockVersion()));
            ...
            assertEquals("description", saved.description()); assertEquals(RequirementStatus.ACTIVE, saved.status());
    ...
    @Test void uniqueForeignKeyAndCheckErrorsAreTranslated() {
        tx.inTransaction(c -> {
            var p = parents(c); var dao = new JdbcRequirementDao(c);
            dao.insert(requirement(p, 1));
            assertEquals(1062, assertThrows(DataAccessException.class, () -> dao.insert(requirement(p, 1))).getVendorCode());
            assertEquals(1452, assertThrows(DataAccessException.class,
                    () -> dao.insert(requirement(new Parents(p.userId(), -1L), 2))).getVendorCode());
            assertEquals(1452, assertThrows(DataAccessException.class,
                    () -> dao.insert(requirement(new Parents(-1L, p.projectId()), 2))).getVendorCode());
            assertEquals(3819, assertThrows(DataAccessException.class, () -> dao.insert(requirement(p, 0))).getVendorCode());
            assertEquals(3819, assertThrows(DataAccessException.class, () -> dao.insert(new Requirement(null, p.projectId(), 3L, " ", null, Priority.LOW, RequirementStatus.ACTIVE, p.userId(), null, null, null))).getVendorCode());
            return null;
        });
    }
    ...
    @Test void absentLookupsAndLockWithoutTransaction() throws Exception {
        try (var c = pool.borrow()) {
            var dao = new JdbcRequirementDao(c);
